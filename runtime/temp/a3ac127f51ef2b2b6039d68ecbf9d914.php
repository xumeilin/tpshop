<?php if (!defined('THINK_PATH')) exit(); /*a:4:{s:45:"./template/pc/rainbow/goods\integralMall.html";i:1540260094;s:59:"D:\wamp64\www\tpshop\template\pc\rainbow\public\header.html";i:1547086768;s:59:"D:\wamp64\www\tpshop\template\pc\rainbow\public\footer.html";i:1540260094;s:65:"D:\wamp64\www\tpshop\template\pc\rainbow\public\sidebar_cart.html";i:1540260094;}*/ ?>
<!DOCTYPE html>
<html>
	<head>
		<meta charset="UTF-8">
					...
					<div class="item">
						...
						<div class="cata-nav-layer">
							...
							<div class="advertisement_down">
								<?php $pid =100+$kr;$ad_position = M("ad_position")->cache(true,TPSHOP_CACHE_TIME)->column("position_id,position_name,ad_width,ad_height","position_id");$result = M("ad")->where("pid=$pid  and enabled = 1 and start_time < 1547193600 and end_time > 1547193600 ")->order("orderby desc")->cache(true,TPSHOP_CACHE_TIME)->limit("5")->select();
...
foreach($result as $key=>$v3):       
    ...
    ?>
							...
							<?php $pid =51;$ad_position = M("ad_position")->cache(true,TPSHOP_CACHE_TIME)->column("position_id,position_name,ad_width,ad_height","position_id");$result = M("ad")->where("pid=$pid  and enabled = 1 and start_time < 1547193600 and end_time > 1547193600 ")->order("orderby desc")->cache(true,TPSHOP_CACHE_TIME)->limit("1")->select();
if(is_array($ad_position) && !in_array($pid,array_keys($ad_position)) && $pid)
{
  M("ad_position")->insert(array(
         "position_id"=>$pid,
					...
					<div class="reco-bouti">
						...
						<div class="reacommque">
							<ul>
                                ...
                                $md5_key = md5("select * from `__PREFIX__goods` where is_on_sale = 1 and exchange_integral > 0 and is_recommend = 1 and is_virtual = 0 limit 5");
                                $result_name = $sql_result_goods = S("sql_".$md5_key);
                                if(empty($sql_result_goods))
                                {                            
                                    $result_name = $sql_result_goods = \think\Db::query("select * from `__PREFIX__goods` where is_on_sale = 1 and exchange_integral > 0 and is_recommend = 1 and is_virtual = 0 limit 5"); 
					...
					<div class="hotchane">
						...
						<div class="hot-change">
							<ul>
                                ...
                                $md5_key = md5("select * from `__PREFIX__goods` where is_on_sale = 1 and exchange_integral > 0 and is_hot = 1 and is_virtual = 0 limit 7");
                                $result_name = $sql_result_goods = S("sql_".$md5_key);
                                if(empty($sql_result_goods))
                                {                            
                                    $result_name = $sql_result_goods = \think\Db::query("select * from `__PREFIX__goods` where is_on_sale = 1 and exchange_integral > 0 and is_hot = 1 and is_virtual = 0 limit 7"); 
                                    S("sql_".$md5_key,$sql_result_goods,1);
                                }    
                              foreach($sql_result_goods as $k=>$goods): ?>
